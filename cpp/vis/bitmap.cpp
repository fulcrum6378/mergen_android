...
void Bitmap::open(const std::string &filename) {
    ...
    for (auto &pixel: pixels) pixel.clear();
    pixels.clear();
        ...
        if (magic.magic[0] != 'B' || magic.magic[1] != 'M') {
            ...
            for (int row = 0; row < dib_info.height; row++) {
                ...
                for (int col = 0; col < dib_info.width; col++) {
                    ...
                    row_data.emplace_back(red, green, blue);
                }
...
bool Bitmap::isImage() {
    ...
    if (height == 0 || pixels[0].size() == 0) return false;

    const int width = pixels[0].size();

    for (int row = 0; row < height; row++) {
        if (pixels[row].size() != width) return false;
